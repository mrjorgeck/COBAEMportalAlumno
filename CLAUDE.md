...
**MVP completo**: Fases 0–3 construidas y verificadas (CI en verde, 46 tests; commits Fase 1.1–1.16, 2.1–2.9, 3.1–3.5). Los 20 criterios de aceptación de §31 están cubiertos: registro, PDF, documentación, CSV, avisos, módulos, evaluación, materiales, propedéutico, segunda evaluación, dashboards, grupo escolar, matrícula (solo captura/import — nunca se genera), horario, SICOBaEM y reutilización multiciclo (aislamiento 2026/2027 probado). Repo en la raíz (github.com/mrjorgeck/COBAEMportalAlumno), CI en `.github/workflows/ci.yml`. Deuda menor anotada: `POST /admin/ciclos` usa permiso `modulos.publicar`; el diseño (docs/03) lo reserva al admin — corregir en la siguiente fase. Siguientes pasos: (1) despliegue a producción (`deploy/hostinger-checklist.md` + `deploy/deploy.sh`) y UAT con control escolar; (2) Fase 4 piloto OMR (`docs/09` §2, diseño en `docs/05`). Pendientes con el plantel en `docs/09` §6 — el aviso de privacidad y el formato PDF oficial BLOQUEAN el arranque real con alumnos.
